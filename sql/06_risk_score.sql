...
-- Scores closer to 1.0 indicate highest risk.
...
-- Preview top 25 highest-risk assets (runs when reading sql/06_risk_score.sql)
SELECT
  truck_id,
  ROUND(risk_score, 4) AS risk_score,
  risk_tier,
  recommended_action,
  ROUND(downtime_per_mile, 8) AS downtime_per_mile
